...
/obj/random
	...
	alpha = 64
	// Determines likelihood that this random object doesn't spawn anything.
...
/obj/random/proc/spawn_item()
	...
	if(spread_range && istype(loc, /turf))
		for(var/turf/T in range(spread_range, src.loc))
			//if(!T.is_wall && !T.is_hole)
			points_for_spawn += T
	else
		points_for_spawn += loc // For spawning objects inside containers.
	for(var/i in 1 to rand(min_amount, max_amount))
		var/build_path = item_to_spawn()
		if(!build_path)
			return list()
		if(!points_for_spawn.len)
			debug_world_log("Spawner \"[type]\" ([x], [y], [z]) tried to spawn without any free space.")
			break
