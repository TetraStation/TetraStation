...
/obj/item/stack/tile/attack_self(mob/user)
	...
	if(tile_reskin_types && tile_rotate_dirs)
		radial_options += list("Reskin" = image(icon = 'icons/mob/radial.dmi', icon_state = "radial_reskin"))
		radial_options += list("Rotate" = image(icon = 'icons/mob/radial.dmi', icon_state = "radial_rotate"))
...
/// Copy the vars of one tile to another, minus merge_type - used for cyborg tiles.
/obj/item/stack/tile/proc/copy_tile(mob/user, var/obj/item/stack/tile/tile_to_copy)
		icon = tile_to_copy.icon // In order to prevent borg tiles from breaking, we copy the appearance and turf type of tiles
		icon_state = tile_to_copy.icon_state
		turf_type = tile_to_copy.turf_type
		name = tile_to_copy.name
		singular_name = tile_to_copy.name
		tile_rotate_dirs = tile_to_copy.tile_rotate_dirs
		desc = tile_to_copy.desc
