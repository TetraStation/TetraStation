...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor
	...
	icon = 'icons/obj/machines/reactor/rbmk.dmi'
	...
	var/common_channel = RADIO_CHANNEL_COMMON

	var/datum/looping_sound/nuclear_reactor/soundloop
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/Initialize(mapload)
	...
	investigate_log("has been created.", INVESTIGATE_SINGULO)
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/slowprocess()
	...
	if(input_moles >= minimum_coolant_level)
		...
		var/heat_delta = (coolant_input.return_temperature() / 273) * gas_absorption_effectiveness
	...
	if(moderator_input.total_moles() >= minimum_coolant_level)
		...
		if(total_fuel_moles >= minimum_coolant_level) //You at least need SOME fuel.
			...
			if(power >= 5)
				...
				coolant_output.gases[/datum/gas/nitryl][MOLES] += total_fuel_moles/100 //Shove out nitryl into the air when it's fuelled. You need to filter this off, or you're gonna have a bad time.
				coolant_output.gases[/datum/gas/radon][MOLES] += total_fuel_moles/nucleium_output //Shove out radon into the air when it's fuelled. You need to filter this off, or you're gonna have a bad time.
		...
		var/total_permeability_moles = moderator_input.gases[/datum/gas/bz][MOLES] + (moderator_input.[/datum/gas/water_vapor][MOLES]*2) + (moderator_input.gases[/datum/gas/hypernoblium][MOLES]*10)
	...
	for(var/atom/movable/atom_on_reactor in get_turf(src))
		if(isliving(atom_on_reactor))
			...
			var/temp_diff = temperature - living_mob.bodytemperature
			if(temp_diff <= 0)
				continue
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/handle_alerts()
	...
	if(temperature >= RBMK_TEMPERATURE_CRITICAL)
		...
		if(temperature >= RBMK_TEMPERATURE_MELTDOWN)
			...
			if(vessel_integrity <= temperature / 100) //It wouldn't be able to tank another hit.
				investigate_log("Reactor melted down at [temperature] K with desired criticality at [desired_k]", INVESTIGATE_SINGULO)
	...
	if(pressure >= RBMK_PRESSURE_CRITICAL)
		...
		var/turf/reactor_turf = get_turf(src)
		reactor_turf.atmos_spawn_air("water_vapor=[pressure/100];TEMP=temperature]")
	...
	if(warning)
		if(!alert) //Congrats! You stopped the meltdown / blowout.
			...
			light_color = LIGHT_COLOR_CYAN
			set_light(10)
			investigate_log("Reactor stabilizing at [pressure] kPa and [temperature] K.", INVESTIGATE_SINGULO)
			message_admins("Reactor stabilizing at [pressure] kPa and [temperature] K. [ADMIN_VERBOSEJMP(src)]")
			radio.talk_into(src, "REACTOR STABILIZED at [pressure] kPa and [temperature] K." , engineering_channel)
	else
		if(!alert)
			return
		...
		relay('sound/machines/reactor/alarm.ogg', null, loop = TRUE, channel = CHANNEL_ENGINE_ALERT)
		set_light(0)
		light_color = LIGHT_COLOR_BLOOD_MAGIC
		set_light(10)
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/meltdown()
	...
	relay('sound/machines/reactor/meltdown.ogg', "<span class='userdanger'>You hear a horrible metallic hissing.</span>")
	stop_relay(CHANNEL_ENGINE_ALERT)
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/blowout()
	...
	relay('sound/machines/reactor/explode.ogg')
	priority_announce("High levels of radiation detected. Maintenance is best shielded from radiation.", "Nuclear Blowout Alert", "Radiation Alert")
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/start_up()
	...
	desired_k = 1
	set_light(10)
	var/startup_sound = pick('sound/machines/reactor/startup.ogg', 'sound/machines/reactor/startup2.ogg')
...
/obj/machinery/atmospherics/components/trinary/nuclear_reactor/proc/shut_down()
	...
	investigate_log("Reactor shutdown at [pressure] kPa and [temperature] K.", INVESTIGATE_SINGULO)
	message_admins("Reactor shutdown at [pressure] kPa and [temperature] K. [ADMIN_VERBOSEJMP(src)]")
	radio.talk_into(src, "REACTOR SHUTDOWN INITIATED at [pressure] kPa and [temperature] K. Venting remaining gasses and stablizating reactions. Please eject fuel rods and make any necessary repairs before restarting the reactor." , engineering_channel)

	playsound(loc, 'sound/machines/hiss.ogg', 100)
	set_light(0)

	sleep(20)
	playsound(loc, 'sound/machines/clockcult/steam_whoosh.ogg', 100, TRUE)
	var/turf/reactor_turf = get_turf(src)
	reactor_turf.atmos_spawn_air("water_vapor=[pressure/10];TEMP=temperature]")
