...
/datum/gas_mixture/proc/equalize(datum/gas_mixture/other)
	...
	for(var/gas_id in gas_list)
		...
		if(temperature == 0)
			temperature = TCMB // temp of space.

		//math is under the assumption temperatures are equal
		if(abs(other.gases[gas_id][MOLES] / other.volume - other.gases[gas_id][MOLES] / other.volume) > min_p_delta / (R_IDEAL_GAS_EQUATION * temperature))
			. = TRUE
